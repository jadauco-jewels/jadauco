...
=IF(A2="","",SWITCH(MID(A2,4,2),"NK","Necklaces","ER","Earrings","BG","Bangles",
  "BR","Bracelets","RG","Rings","PD","Pendants","⚠ unknown code"))
```
...
|---|---|
| **Product Code** | `JD-NK-001`. The two letters in the middle decide the category — `NK` necklaces, `ER` earrings, `BG` bangles, `BR` bracelets, `RG` rings, `PD` pendants. Must be unique; no two products share a code. The `Category (auto)` column next to it tells you which one you picked. |
